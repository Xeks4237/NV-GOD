return {
	...
	config = function()
		...
		dashboard.section.buttons.val = {
			...
			), -- Allows to search files in directory where you opened nvim
			dashboard.button(
				'g',
				'  Find Text',
				'<cmd>Telescope live_grep<CR>'
			), -- Allows to Search text in files using live_grep of telescope plugin
			dashboard.button(
				'o',
				'  Recent Files',
				'<cmd>Telescope oldfiles<CR>'
			), -- Allows to Search files which are were opened before using telescope plugin
			...
			), -- Shows popup of which-key plugin with most the keymaps
			dashboard.button(
				'c',
				'  Config',
				'<cmd>Telescope find_files cwd=~/.config/nvim<CR>'
			), -- Allows to Search files in directory with nvim configs
